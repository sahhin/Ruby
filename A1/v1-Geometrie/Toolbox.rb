...
class TkMethodenDialog < TkToplevel
  ...
  def initialize(parent,toolbox, instanz, instanz_symbol, methoden_name, &block)
      ...
      labels_and_types.each_with_index  do |label_and_type, index|
       ...
       ok_button = Tk::Tile::Button.new(self) {text $OK ; default "active"
               command proc {
                   entry_vals  = [];
                   entries.each_index{|index|
                     entry = entries[index]
                     type = labels_and_types[index][1]
                     if (entry.get().empty?())  # ADDED 13.3.2019
                       return
                     end
                     if type == :String
            ...
            ok_button.grid(:column => 0, :row => row)
