...
class Turtle
  ...
  def initialize(x,y,angle = 0,color=:black)
    super()
    @pos = Point.new(x,y)
  ...
  def go_ahead(steps,width=1,color=@color)
    x0 = @pos.x
    y0 = @pos.y
    ...
    if (@down)
      animiere(3) {
        TkcLine.new( Leinwand.gib_einzige_instanz(),x0,y0,x1,y1,:fill => color, :width => [1,width] .max())
       }
    end
    @pos = Point.new(x1,y1)
  end
